...
whiptail --title "Jorink's Arch-Hyprland (2025) Install Script" \
    --msgbox "Welcome to Jorink's Arch-Hyprland (2025) Install Script!!!\n\n\
...
echo "👌 ${OK} 🇵🇭 ${MAGENTA}Jorink's..${RESET} ${SKY_BLUE}lets continue with the installation...${RESET}" | tee -a "$LOG"
...
options_command+=(
    ...
    "dots" "Download and install pre-configured Jorink's Hyprland dotfiles?" "OFF"
)
...
while true; do
    ...
    if [[ "$dots_selected" == "OFF" ]]; then
        ...
        if ! whiptail --title "Jorink's Hyprland Dot Files" --yesno \
        "You have not selected to install the pre-configured Jorink's Hyprland dotfiles.\n\nKindly NOTE that if you proceed without Dots, Hyprland will start with default vanilla Hyprland configuration and I won't be able to give you support.\n\nWould you like to continue install without Jorink's Hyprland Dots or return to choices/options?" \
    ...
    echo "👌 ${OK} You confirmed your choices. Proceeding with ${SKY_BLUE}Jorink's 🇵🇭 Hyprland Installation...${RESET}" | tee -a "$LOG"
    break  
done
...
echo "${INFO} Installing ${SKY_BLUE}Jorink's Hyprland additional packages...${RESET}" | tee -a "$LOG"
sleep 1
...
for option in "${options[@]}"; do
    case "$option" in
        ...
        rog)
            ...
            ;;
        dots)
            echo "${INFO} Installing pre-configured ${SKY_BLUE}Jorink's Hyprland dotfiles...${RESET}" | tee -a "$LOG"
...
if pacman -Q hyprland &> /dev/null || pacman -Q hyprland-git &> /dev/null; then
    ...
    printf "${SKY_BLUE}Thank you${RESET} 🫰 for using 🇵🇭 ${MAGENTA}Jorink's's Hyprland Dots${RESET}. ${YELLOW}Enjoy and Have a good day!${RESET}"
    printf "\n%.0s" {1..2}
